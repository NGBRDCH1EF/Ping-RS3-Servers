...
def pingall(valid_worlds):
    ...
    for world in valid_worlds:
        results[world] = ping_world(world)
    return results
...
def ping_world(world):
    ...
    result = subprocess.run(
        ["ping", count_flag, "2", host],  # 4 pings to get stats
        capture_output=True,
        text=True
    )
    ...
    if platform.system().lower() == "windows":
        # Windows summary line: "Minimum = 22ms, Maximum = 30ms, Average = 25ms"
        ...
        # Linux/macOS: "rtt min/avg/max/mdev = 22.123/25.456/30.789/1.234 ms"
...
# def find_unreachable(start=1, end=259):
#     unreachable = []
#     for world in range(start, end + 1):
#         print(f"Checking world {world}...")
#         if not ping_world(world):
#             unreachable.append(world)
#     return unreachable


# Run across all 259 worlds
# bad_worlds = find_unreachable(1, 259)
# print(bad_worlds)
# print(len(bad_worlds))
while menu():
    pass
